...
private:
    ...
    std::map<std::string, Person*> currentPatients_;
    std::map<std::string, Person*> staff_;

    // Contains data of every patient that has ever been in the hospital.
    // { name of patient, { pointer to 'patient', every Care period pointer } }
    std::vector<std::pair<std::string,
             std::pair<Person*, std::vector<CarePeriod*>>>> patientDatabase_;
};
